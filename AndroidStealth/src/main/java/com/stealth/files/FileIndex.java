...
/**
 * This class provides all the functionality to manage all files in the app. It virtually has
 * folders and files with nice names, but in real the files are all stored in the same
 * physical folder with not-so-nice names (UUID's)
...
public class FileIndex
{
    ...
    private FileIndex(boolean fake, final IOnResult<FileIndex> callback) {
        EncryptedPreferences.get(
                fake ? PREFERENCES_KEY_FAKE : PREFERENCES_KEY_REAL,
                new IOnResult<EncryptedPreferences>()
                {
                    @Override
                    public void onResult(EncryptedPreferences result)
                    {
                        Utils.toast("Created the EncryptedPreferences");
                        mPreferences = result;
                        ...
                        try
                        {
                            ...
                            {
                                Utils.toast("Index exists.. let's read and parse");
                            ...
                            {
                                Utils.toast("Index does not exist.. let's create, parse and make the root");
    ...
    /**
     * Saves changes by committing the preferences file
     * and notifying our listeners.
     */
    ...
    public void addFile(IndexedFile file) {
        Utils.toast("old length " + mFilesArray.length());
        file.setJsonID(mFilesArray.length());
        mFilesArray.put(file.getRaw());
        Utils.toast("new length " + mFilesArray.length());
    ...
    public void removeFolder(IndexedFolder folder) {
        ...
        // remove recursively
        for (IndexedFolder f : folder.getFolders()) removeFolder(f);
        for (IndexedFile f : folder.getFiles()) removeFile(f);

        if (folder != mRoot) // we can never remove the root!
        {
            int i = folder.getJsonID();
            JSONArray movedItem = (JSONArray) mFoldersArray.remove(i);
            if (movedItem != null) getFolder(movedItem).setJsonID(i); // remember its new position
    ...
    public void removeFile(IndexedFile file)
    {
        ...
        JSONArray movedItem = (JSONArray) mFilesArray.remove(i);
        if (movedItem != null) getFile(movedItem).setJsonID(i); // remember its new position
